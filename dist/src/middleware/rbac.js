    ...
    return (req, res, next) => {
        try {
            ...
            if (!allowedRoles.includes(req.user.role)) {
                throw new AppError(`Access denied. Required role: ${allowedRoles.join(' or ')}`, 403);
            }
            next();
        }
        catch (error) {
 ...
 */
export const requireCompanyAdminOrSuperAdmin = requireRole(UserRole.COMPANY, UserRole.SUPERADMIN);
/**
 * Check if user is either employee or company admin
 */
export const requireEmployeeOrCompanyAdmin = requireRole(UserRole.EMPLOYEE, UserRole.COMPANY);
/**
 ...
 */
export const requireLocationAccess = (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 401);
        }
