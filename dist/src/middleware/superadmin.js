...
export const requireSuperAdmin = (req, res, next) => {
    try {
        ...
        if (req.user.role !== UserRole.SUPERADMIN) {
            throw new AppError('Access denied. This action requires superadmin privileges.', 403);
        }
        next();
    }
    catch (error) {
    ...
    try {
        ...
        if (req.user && req.user.role !== UserRole.SUPERADMIN) {
            ...
            if (req.body.role === UserRole.SUPERADMIN) {
                throw new AppError('Cannot assign superadmin role. Only superadmins can create other superadmins.', 403);
            }
        }
        next();
    }
